 ...
 *   2. 방문 배열을 [26]개의 일차원 배열 대신 bitmask 이용
 ...
 *   2. (0, 0)좌표에서 이동 거리를 1로 해서 dfs로 탐색한다. 현재 알파벳도 count한다. 
...
public class Main_1987_G4_알파벳2 {
    ...
    static int R, C, max;
    static int[][] map;
    static int[][] visited; // [r][c]에 여태까지 방문한 알파벳은 bitmask로 기록
    ...
    private static void dfs(int r, int c, int bit, int cnt) {
        ...
        if (max == 26)
            return;
        visited[r][c] = bit; // 시작 좌표에서 현재 좌표까지 방문한 알파벳을 메모한다.

        for (int i = 0; i < 4; i++){
            int nr = r + dr[i];
            int nc = c + dc[i];
            if(nr<0 || nr>=R || nc<0|| nc>=C) continue;
            // 방문 체크(현재 좌표의 알파벳이 이미 방문한 알파벳인지 확인)
            if ((bit & 1 << map[nr][nc]) == 0 &&
                    // map[nr][nc]위치의 알파벳을 bit에 추가한 후 이전 기록과 같은지 체크
                    (bit | 1 << map[nr][nc]) != visited[nr][nc]) { // 같으면 안가고 ==> 탐색해봤자 같은 경우의 수이기 때문

...
2. `findset(x)` : 원소 x의 root를 찾는 함수

   - **path compression**: x의 root를 따라가면서 root를 찾는다.
   - **union by rank**: x의 root와 y의 root를 비교하여 rank가 더 큰 집합의 root를 작은 집합의 root로 만든다.
   - 시간복잡도: O(n)
